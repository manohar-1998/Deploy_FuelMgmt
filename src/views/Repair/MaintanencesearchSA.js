...
import { deleteMaintanence, getMaintenanceSheetforSA } from 'src/Apicalls';
toast.configure();
function MaintanencesearchSA() {
    ...
    const preload = (apiUrl, filter) => {
        getMaintenanceSheetforSA(apiUrl, filter).then((values) => {
            console.log("ApiUrl check, Filter check=", values)
            if (values.error) {
                console.log(values.error);
            } else {
                setOrder(values.data.MaintenanceSheet);
    ...
    const deleteConfirmed = (id) => {
        ...
        if (answer) {
            deleteMaintanence(id).then((data) => {
                if (data.error) {
                            ...
                            {order.map((ord, index) => {
                                return (
                                    <Table.Row key={index}>
                                        ...
                                        <Table.Cell>{ord.Category}</Table.Cell>
                                        <Table.Cell>{ord.Date}</Table.Cell>
                                        <Table.Cell>
                                            <Header as="h5" color={ord.Status === 'Complete' ? 'green' : 'red'}>{ord.Status} </Header>
                                        </Table.Cell>
                                        <Table.Cell>{ord.FaultDescription} </Table.Cell>
                                        <Table.Cell >
                                            <Link to={`/EditMaintanence/${ord._id}`}>
                                                <Button
                                                    content="Edit"
                                                    icon="edit"
                                                    color="blue"
...
export default MaintanencesearchSA;
